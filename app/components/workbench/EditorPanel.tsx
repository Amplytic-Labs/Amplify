...
export const EditorPanel = memo(
  ({
    ...
    const handleVersionSelect = useCallback(
      (versionIndex: number) => {
        if (!editorDocument || !fileHistory) {
          return;
        }

        const history = fileHistory[editorDocument.filePath];

        if (!history || !history.versions[versionIndex]) {
          return;
        }
      ...
      <>
                  ...
                  {activeFileSegments?.length && (
                    <div className="flex items-center flex-1 text-sm">
                      ...
                      <div className="flex gap-1 ml-auto -mr-1.5">
                        ...
                        {/* View Toggle */}
                        {editorDocument &&
                          ['.md', '.mermaid', '.svg', '.chartjs'].some((ext) =>
                            editorDocument.filePath.endsWith(ext),
                          ) && (
        ...
        <DialogRoot open={memoryOpen} onOpenChange={setMemoryOpen}>
          <Dialog
            ...
            onBackdrop={() => setMemoryOpen(false)}
            ...
            <div className="flex flex-col max-h-[85vh] overflow-hidden">
              <div className="flex-1 overflow-y-auto custom-scrollbar">
                {projectId ? (
                  ...
                  <div className="p-6 text-sm text-amplify-elements-textSecondary">No project linked to this chat.</div>
                )}
              </div>
            </div>
          </Dialog>
        ...
        <DialogRoot open={historyOpen} onOpenChange={setHistoryOpen}>
          <Dialog
            ...
            onBackdrop={() => setHistoryOpen(false)}
            className="w-[560px] max-w-[92vw] max-h-[85vh]"
          >
            <div className="flex flex-col max-h-[85vh] overflow-hidden">
              <div className="flex-1 overflow-y-auto custom-scrollbar">
                {projectId ? (
