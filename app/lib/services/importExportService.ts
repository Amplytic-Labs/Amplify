...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          amplify_user_profile: this._safeGetItem('amplify_user_profile'),
          amplify_settings: this._safeGetItem('amplify_settings'),
          amplify_profile: this._safeGetItem('amplify_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('amplify_viewed_features'),
          developer_mode: this._safeGetItem('amplify_developer_mode'),
        ...
        ui: {
          // Tab configuration
          amplify_tab_configuration: this._safeGetItem('amplify_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('amplify_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('amplify_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          amplify_read_logs: this._safeGetItem('amplify_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('amplify_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('amplify_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.amplify_tab_configuration) {
        try {
          this._safeSetItem('amplify_tab_configuration', data.ui.amplify_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'amplify_acknowledged_debug_issues',
        'amplify_acknowledged_connection_issue',
        'error_logs',
        'amplify_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('amplify_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
