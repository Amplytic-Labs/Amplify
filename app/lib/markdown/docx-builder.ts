import {
  ...
  PageOrientation, LevelFormat, ShadingType, convertInchesToTwip,
...
export async function buildDocx(
  ...
  if (ctx.forPreview) return buffer;
...
async function renderList(node: any, ctx: BuildContext, depth: number): Promise<any[]> {
  ...
  const orderedChar = (n: number) => {
    if (depth === 0) return `${n}.  `;
    if (depth === 1) return `${toAlpha(n)}.  `;   // a. b. c.
...
async function buildTable(node: any, ctx: BuildContext): Promise<Table> {
  ...
  const INLINE_TYPES = new Set([
    "text", "strong", "emphasis", "delete", "inlineCode",
    "link", "image", "break", "inlineMath",
  ]);
  for (let r = 0; r < node.children.length; r++) {
    ...
    for (const tc of tr.children) {
      ...
      cells.push(new TableCell({
        children: paras,
        shading: isHeader
          ? { type: ShadingType.CLEAR, fill: ctx.theme.tableHeaderBg, color: "auto" }
          : undefined,
...
async function injectOmml(buffer: Buffer, ommlMap: Map<string, string>): Promise<Buffer> {
  if (ommlMap.size === 0) return buffer;
