...
import {
  Paragraph, TextRun, AlignmentType, BorderStyle, ShadingType,
  convertInchesToTwip,
} from "docx";
...
export async function renderHtmlBlock(html: string, theme?: ResolvedDocxTheme): Promise<Paragraph[]> {
  ...
  const root = parse(html, {
    blockTextElements: {
      script: false, style: false, pre: true,
    },
  });
...
function collectInlineRuns(nodes: any[], style: RunStyle, rctx: HtmlRenderCtx): TextRun[] {
  ...
  for (const node of nodes) {
    ...
    if (node.nodeType === NodeType.TEXT_NODE) {
      ...
      if (text) out.push(new TextRun(applyStyle({ text }, style)));
      continue;
    }
    ...
    switch (tag) {
      ...
      case "span":
        ...
        if (node.attributes.color) childStyle.color = sanitizeColor(node.attributes.color);
        if (node.attributes.style) {
          const m = node.attributes.style.match(/color:\s*([^;]+)/);
          if (m) childStyle.color = sanitizeColor(m[1].trim());
        }
        break;
      case "font":
        if (node.attributes.color) childStyle.color = sanitizeColor(node.attributes.color);
