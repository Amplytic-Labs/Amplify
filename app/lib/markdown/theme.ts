...
export function sanitizeColor(c: string | null | undefined): string | null {
  ...
  if (rgbMatch) {
    const [r, g, b] = [rgbMatch[1], rgbMatch[2], rgbMatch[3]].map((v) => parseInt(v, 10).toString(16).padStart(2, '0'));
 ...
 */
function applyThemeField(theme: DocxTheme, key: string, strVal: string | undefined, numVal: number | undefined): void {
  if (COLOR_THEME_FIELDS.has(key)) {
    if (strVal) {
      (theme as any)[key] = strVal;
    }
  } else if (NUM_THEME_FIELDS.has(key)) {
    if (numVal !== undefined && isFinite(numVal)) {
      (theme as any)[key] = numVal;
    }
